...
*/
		...
		$mc_template_id = get_post_meta($post->ID, '_mc_template_id', true);
		
		echo '<p>Mailchimp ID:' . $mc_template_id . '</p>';
	...
	/* New Updates For Parsing Ends Here */
		...
		//$mc_name = get_post_meta($post->ID, '_mc_name', true);
		//if ($mc_name){
		//	echo '<label class="det">Mailchimp Template Name</label>';
		//	wp_editor($mc_name, 'mc_name', $SL_settings);
		//}
		...
		*/
		...
		$GE_newsvalue_top = get_post_meta($post->ID, '_GE_news_box1_top', true);
		if ($GE_newsvalue_top){
			echo '<hr id="Global_Espresso">';
			
			$GE_quote_english = get_post_meta($post->ID, '_GE_quote_english', true);
			$GE_quote_arabic = get_post_meta($post->ID, '_GE_quote_arabic', true);
			
			echo '<label class="det">Global Espresso English Quote</label>';
			wp_editor($GE_quote_english, 'GE_quote_english', $settings);

			echo '<label class="det">Global Espresso Arabic Quote</label>';
			wp_editor($GE_quote_arabic, 'GE_quote_arabic', $settings);

/* 			$mc_name = get_post_meta($post->ID, '_mc_name', true);
			echo '<label class="det">Mailchimp Template Name</label>';
			wp_editor($mc_name, 'mc_name', $settings);
 */	
...
*/
	...
	*/
		...
		if (isset($_POST['GE_news_box_1_top'])) {
			for ($GE_new_no=1; $GE_new_no <= 5; $GE_new_no ++){
		...
		if (isset($_POST['KP_news_box_1_top'])) {
			for ($KP_new_no=1; $KP_new_no <= 4; $KP_new_no ++){
		...
		if (isset($_POST['kp_ln_news_box_1_top'])) {
			for ($new_no=1; $new_no <= 11; $new_no ++){
		...
		$GE_quote_english = $_POST['GE_quote_english'];
		update_post_meta($ID, '_GE_quote_english', $GE_quote_english);
		
		$GE_quote_arabic = $_POST['GE_quote_arabic'];
		update_post_meta($ID, '_GE_quote_arabic', $GE_quote_arabic);
...
/*					// Kingdom Pulse Parsing
			...
			// Extract Podcast intro (English and Arabic) from the "Podcast:" line
			foreach ($sections as $section) {
				if (stripos($section, 'Podcast:') !== false) {
					$introText = trim(preg_replace('/^.*?Podcast:\s*/i', '', $section));
					if (preg_match('/^(.*?)([\x{0600}-\x{06FF}].*)$/us', $introText, $introMatches)) {
						$output['intro_english'] = trim(trim($introMatches[1]), '"');
						$output['intro_arabic']  = trim($introMatches[2]);
					} else {
						$output['intro_english'] = trim(trim($introText), '"');
					}
					break;
				}
			}
			...
			foreach ($global_sections as $section) {
				...
				// Strip newsletter template markers that may trail into the last topic's Arabic content
				$arabic_full = preg_replace('/(?:KINGDOM PULSE|نبض المملكة|TOP TALKS|LOCAL NEWS|ON THE GLOBAL FRONT).*/is', '', $arabic_full);
				$arabic_full = trim($arabic_full);
	...
	function createMailchimpTemplate($apiKey, $templateName, $htmlContent) {
		// Extract the data center prefix from the API key
		list(, $dc) = explode('-', $apiKey);
		$url = "https://{$dc}.api.mailchimp.com/3.0/templates";
		...
		if ($httpCode == 200) {
			...
			return $responseData['id']; // Return the template ID
		} else {
			// Handle errors
			error_log ("Error creating template: HTTP Status Code {$httpCode}");
	...
	/**
	 * Update an existing Mailchimp template's HTML, disabling the Expect:100-continue handshake.
	 *
	 * @param string $apiKey        Your Mailchimp API key (e.g. “abcd1234-us19”).
	 * @param int    $templateId    The ID of the template to update.
	 * @param string $htmlContent   The full HTML string to set as the template’s content.
	 * @return bool|int             Returns the template ID on success, or false on failure.
	 */
	function updateMailchimpTemplate($apiKey, $templateId, $htmlContent) {
		// Extract data center prefix from the API key
		list(, $dc) = explode('-', $apiKey);
		$url = "https://{$dc}.api.mailchimp.com/3.0/templates/{$templateId}";

		// Prepare the JSON payload
		$payload = json_encode(['html' => $htmlContent]);
		...
		curl_setopt($ch, CURLOPT_USERPWD, "anystring:{$apiKey}");
		// Tell cURL not to wait for 100-continue
		...
		if ($httpCode === 200) {
			...
			return $resp['id'] ?? $templateId;
		}

		// Log and return failure
		error_log("Mailchimp template update failed (HTTP {$httpCode}): {$response}");
		return false;
	}

	/**
	 * Convert a positive integer (or numeric string) into Eastern‑Arabic numerals.
	 *
	 * @param string|int $number
	 * @return string
	 */
	function to_eastern_arabic_numeral($number) {
		// Map Western digits to Eastern‑Arabic digits
		static $map = [
			'0' => '٠','1' => '١','2' => '٢','3' => '٣','4' => '٤',
			'5' => '٥','6' => '٦','7' => '٧','8' => '٨','9' => '٩',
		];
		// Cast to string and replace each Western digit with its Arabic counterpart
	 ...
	 *  - {{#section}}…{{/section}} loops
	 *  - {{var}} single placeholders
	 */
	 /*
	function render_template( $template, array $data ) {
	  ...
	  $template = preg_replace_callback(
		'/\{\{#([^}]+)\}\}(.*?)\{\{\/\1\}\}/s',
		function($m) use ($data) {
			$section = $m[1];        // e.g. "top_talks"
			$innerTpl = $m[2];       // HTML between the tags
			$out = '';
			...
			if (!empty($data[$section]) && is_array($data[$section])) {
				foreach ($data[$section] as $i => $item) {
					$idx = $i + 1;
				    $item['index'] = $idx;
				    $item['index_ar'] = to_eastern_arabic_numeral($idx);

				    // Now replace {{key}} placeholders in the inner template
				    $row = $innerTpl;
				    foreach ($item as $k => $v) {
	  ...
	  // 2) Handle single placeholders {{var}}
	...
	*/
	...
	/**
	 * Phase A: one‐shot section expansion
	 */
	function render_sections($template, array $data) {
		return preg_replace_callback(
			'/\{\{#([^}]+)\}\}(.*?)\{\{\/\1\}\}/s',
			function($m) use ($data) {
				$section  = $m[1];
				...
				// 1) Named array section, e.g. "topics" → repeat
				if (isset($data[$section]) && is_array($data[$section])) {
					...
					foreach ($data[$section] as $i => $item) {
						...
						$item['index'] = $idx;
						$item['index_ar'] = to_eastern_arabic_numeral($idx);
						//$item['index_ar'] = $item['index_ar'] ?? '';
						// For each numeric subsection inside innerTpl we want only one pass,
						// so call a small helper that handles numeric-only loops + variable
						$out .= render_one_level($innerTpl, $item);
					}
					return $out;
				}

				// 2) Numeric section, e.g. "0" or "1" → single item
				if (ctype_digit($section)
					&& isset($data[$section])
					&& is_array($data[$section])
				) {
					return render_one_level($innerTpl, $data[$section]);
				}

				// 3) Nothing matched → strip it
				return '';
			},
			$template
		);
	...
	/**
	 * Phase A helper: strips out any numeric section loops (0/1) and
	 * then does a Phase B variable pass on the remaining string.
	 */
	function render_one_level($tpl, array $ctx) {
		// remove numeric loops
		$tpl = preg_replace_callback(
			...
			function($m) use ($ctx) {
				...
				if (isset($ctx[$key]) && is_array($ctx[$key])) {
					// Now just a single variable pass
					return render_vars($inner, $ctx[$key]);
				}
				return '';
			},
			$tpl
		);
		// then do variable replacement on what's left
		return render_vars($tpl, $ctx);
	}

	/**
	 * Phase B: variable-only replacement {{var}}
	 */
	function render_vars($template, array $data) {
		return preg_replace_callback(
			'/\{\{(\w+)\}\}/',
			function($m) use ($data) {
				return isset($data[$m[1]]) ? $data[$m[1]] : '';
	...
	/**
	 * Master renderer: sections → variables
	 */
	function render_template($template, array $data) {
		// Phase A: strip out all sections and numeric loops
		$step1 = render_sections($template, $data);
		// Phase B: final variable pass (any top-level {{var}})
		return render_vars($step1, $data);
	}

	
	/**
	 * Given a date string (Y-m-d or DateTime), return which Tuesday of the month it is.
	 *
	 * @param string|\DateTime $date  The date to test (e.g. '2025-05-13' or a DateTime object).
	 * @return int  1 for first Tuesday of that month, 2 for second, etc.  Returns 0 if the date is not a Tuesday.
	 */
	function get_tuesday_week_no( $date ) {
		...
		// If it’s not a Tuesday, return 0 (or you could throw, up to you)
		// ISO-8601 weekday numbers: Monday=1 … Sunday=7, so Tuesday=2
		if ( (int)$dt->format('N') !== 2 ) {
			return 0;
		}

		// Build iterate from the 1st of this month to $dt,
		// counting how many Tuesdays we've seen.
		$year  = $dt->format('Y');
		$month = $dt->format('m');
		$first = new DateTime("{$year}-{$month}-01");
		$count = 0;

		// Walk day by day until (and including) our target date
		while ( $first <= $dt ) {
			if ( (int)$first->format('N') === 2 ) {
				$count++;
			}
			$first->modify('+1 day');
		}
	 ...
	 */
 	function parse_and_save_newsletter($post_id) {
		...
		if ( isset($_POST['newsletter_text']) ) {
			...
			$postdate_d = get_the_date('d', $post_id);
			$postdate_d2 = get_the_date('D', $post_id);
			$postdate_m = get_the_date('M', $post_id);
			$postdate_y = get_the_date('Y', $post_id);                                
			$post_date_a = single_post_arabic_date($postdate_d,$postdate_d2, $postdate_m, $postdate_y);

			if ($newsletter_type === "morning_talks") {
				/* Start Mailchimp Updates */
				$filename = __DIR__ . '/test.html';
				$template_filename = __DIR__ . '/templates/mailchimpmttemplate.html';

				$feat_img = get_the_post_thumbnail_url( $post_id, 'full' );

				// If there's no featured image, you could supply a default or leave it empty.
				if ( !$feat_img ) {
					$feat_img = '';
				}
				...
				$categoryMapa = [
				  'Economy'     => 'الاقتصاد',
				  'M&A'        => 'اندماجات واستحواذات',
				  'Industrial'  => 'الصناعات',
				  'Geopolitical'=> 'جيوسياسية',
				];

				// Enrich the 'talks' array in‑place:
				foreach ( $parsedData['talks'] as &$talk ) {
					$engCat = $talk['category'];         // e.g. "Economy"
					$orgengCat = $engCat;
					
					$index  = $talk['id'];               // e.g. 1,2,…

					$talk['category'] = $categoryMape[ $engCat ] ?? '';
					
					// 1) Lookup the Arabic category label:
					$talk['category_ar'] = $categoryMapa[ $engCat ] ?? '';
					
					$engCat = $talk['category'];
					
					$talk['category'] = $orgengCat;

					// 2) Safely pull the Arabic title from the detailed section:
					if ( isset( $parsedData[ $engCat ][ $index - 1 ] ) ) {
						$talk['title_ar'] = $parsedData[ $engCat ][ $index - 1 ]['title_arabic'];
					} else {
						$talk['title_ar'] = '';
					}
				}
				...
				$data = [
				  ...
				  'post_date'   => $post_date,
				  'post_date_a' => $post_date_a,
				  'featured_image_url'  => $feat_img,
				  'top_talks'  => $parsedData['talks'],       // array of ['title','category','id']
				  'Economy'    => $parsedData['Economy'],     // array of items with title_english, details_english, etc.
				  'M&As'       => $parsedData['M&As'],
				  'Industries' => $parsedData['Industries'],
				  'Geopolitical'=> $parsedData['Geopolitical'],
				];
								
				$htmlContent = file_get_contents($template_filename);
				$htmlContent = render_template($htmlContent, $data);
				
				$write_result = file_put_contents($filename,$htmlContent);
...
/*				if (is_readable($filename)) {
					...
					error_log ("Error: Unable to read the file '{$filename}'.");
				}
*/				
				$mc_name = get_post_meta($post_id, '_mc_name', true);
				if ($mc_name){

					$apiKey = 'YOUR_MAILCHIMP_API_KEY';
					
					// e.g. get_the_date('j F Y') returns "12 April 2025" if that’s when you published
					//$templateName = "Morning Talks {$post_date}";
					$templateName = $mc_name;

					// $templateName = 'Morning Talks Test Template' . the_date('d M Y');

					$templateId = get_post_meta($post_id, '_mc_template_id', true);
					if ($templateId) {
						if (!updateMailchimpTemplate($apiKey, $templateId, $htmlContent)){
							error_log ("Can't update Template. ID: {$templateId}");
						}else{
							error_log ("Template updated successfully. ID: {$templateId}");
						}
					}else{
						$templateId = createMailchimpTemplate($apiKey, $templateName, $htmlContent);
						if ($templateId) {
							update_post_meta($post_id, '_mc_template_id', $templateId);
							error_log ("Template created successfully. ID: {$templateId}");
						}
					}
				}
				...
				$flat = $parsedData['local'];

				// Build paired list
				$paired = [];
				for ($i = 0; $i < count($flat); $i += 2) {
					$eng = $flat[$i];
					$ara = isset($flat[$i+1]) ? $flat[$i+1] : ['title'=>'','details'=>''];

					$paired[] = [
					  'index_e'            => ($i/2) + 1,
					  'index_a'            => to_eastern_arabic_numeral(($i/2) + 1),
					  'title_english'    => $eng['title_english'],
					  'details_english'  => $eng['details_english'],
					  'title_arabic'     => $ara['title_arabic'],
					  'details_arabic'   => $ara['details_arabic'],
					];
				}
				// error_log("paired: " . print_r($paired, true));
				
				$count     = count($paired);
				$perColumn = (int) ceil($count / 2);

				// Left half: topics 1 through ceil(N/2)
				// Right half: topics ceil(N/2)+1 through N
				
				$local_left = array_slice($paired, 0, $perColumn);
				$local_right = array_slice($paired, $perColumn);

				//error_log("local_left: " . print_r($local_left, true));
				//error_log("local_right: " . print_r($local_right, true));

				$filename = __DIR__ . '/test.html';
				$template_filename = __DIR__ . '/templates/mailchimpkptemplate.html';
				...
				// Enrich the 'talks' array in‑place:
				foreach ( $parsedData['top_talks'] as &$talk ) {
					$engCat = $talk['category'];         // e.g. "Local"
					$orgengCat = $engCat;
					
					$index  = $talk['index'];               // e.g. 1,2,…
					...
					if ($engCat == "global"){
						...
						if ( isset( $paired[ $index - 1 ] ) ) {
							$talk['title_ar'] = $paired[ $index - 1 ]['title_arabic'];
						} else {
							$talk['title_ar'] = '';
						}
					}
				...
				unset( $talk );
				$index = 1;
				foreach ( $parsedData['global'] as $i => &$global ) {
					//$index = $global['id']; // 1-based

					// Pull the image URL from post meta:
					$img_url = get_post_meta( $post_id, "_KP_news{$index}_img", true );
					//error_log ("img_url: {$img_url}");
					
					// Fallback or sanitize if needed
					if ( $img_url ) {
						$global['img'] = esc_url( $img_url );
					} else {
						$global['img'] = ''; 
					}
					$index++;
				}
				unset( $global ); // break reference
				
				$data = [
				  'post_link'   => get_permalink($post_id),
				  'post_date'   => $post_date,
				  'post_date_a' => $post_date_a,
				  'top_talks'  => $parsedData['top_talks'],       // array of ['title','category','index']
				  'Local'    => $parsedData['local'],     // array of items with title_english, details_english, etc.
				  'Global' => $parsedData['global'],
				  'local_left' => $local_left,
				  'local_right' => $local_right,
				];
				// error_log("Global: " . print_r($parsedData['global'], true));
				...
				if ($mc_name){
					...
					// e.g. get_the_date('j F Y') returns "12 April 2025" if that’s when you published
					//$templateName = "Kingdom Pulse {$post_date}";
					$templateName = $mc_name;
				...
				// Update the intro paragraph fields from the parsed Podcast intro
				update_post_meta($post_id, '_kp_local_news',   sanitize_text_field($parsedData['intro_english'] ?? ''));
				update_post_meta($post_id, '_a_kp_local_news', sanitize_text_field($parsedData['intro_arabic']  ?? ''));
				...
				$GE_quote_english = get_post_meta($post_id, '_GE_quote_english', true);
				$GE_quote_arabic = get_post_meta($post_id, '_GE_quote_arabic', true);

				$index = 1;
				
				foreach ($parsedData['topics'] as $i => &$topic ) {
					// $topic['index'] = $i + 1;
					// Pull the image URL from post meta:
					$img_url = get_post_meta( $post_id, "_GE_news{$index}_img", true );
					//error_log ("img_url: {$img_url}");

					// Fallback or sanitize if needed
					if ( $img_url ) {
						$topic['img'] = esc_url( $img_url );
					} else {
						$topic['img'] = ''; 
					}
					$topic['post_link'] = get_permalink($post_id);
					$index ++;
				}
				unset( $topic ); // break reference
				
				$alltopics = array_chunk($parsedData['topics'], 2);

				$filename = __DIR__ . '/test.html';
				$template_filename = __DIR__ . '/templates/mailchimpgetemplate.html';
				
				//error_log("alltopics: " . print_r($alltopics, true));
				
				$data = [
				  'quote_english'=> $GE_quote_english,
				  'quote_arabic' => $GE_quote_arabic,
				  'post_link'    => get_permalink($post_id),
				  'post_date'    => $post_date,
				  'post_date_a'  => $post_date_a,
				  'week_no' 	 => get_tuesday_week_no( $post_date ),
				  'topics'		 => $alltopics,
				];
				...
				if ($mc_name){
					...
					// e.g. get_the_date('j F Y') returns "12 April 2025" if that’s when you published
					//$templateName = "Global Espresso {$post_date}";
					$templateName = $mc_name;
	...
	*/
	...
	/*
	** ==========================================
	**  Custom Meta Box : MC-Name - in : Insights
	** ==========================================
	*/

  	function add_mc_name() {
		add_meta_box('MC_name', 'Mailchimp Template Name', 'mc_name', 'insights', 'side');
	}

	function mc_name($post) {
		wp_nonce_field('mc_name', '_mc_name');
		$value = get_post_meta($post->ID, '_mc_name', true);
		echo "<input type='text' style='width: 100%;' id='mc_name' name='mc_name' value='".$value."'>";
	}

	function mc_name_save($ID) {

		if (!isset($_POST['_mc_name'])) {
			return;
		}

		if (!wp_verify_nonce($_POST['_mc_name'], 'mc_name')) {
			return;
		}
		...
		$data = sanitize_text_field($_POST['mc_name']);

		update_post_meta($ID, '_mc_name', $data);
	}

	add_action('add_meta_boxes', 'add_mc_name');
	add_action('save_post', 'mc_name_save');
