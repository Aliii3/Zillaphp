<?php

class CustomSectionV2 extends WP_Widget {

  	public function __construct() {
    	$widget_options = array(
			'classname' => 'CustomSectionV2',
    		'description' => 'Custom section contains 2 cards with call to actions'
    	);
    	parent::__construct(
      		'CustomSectionV2',
      		'WK - Custom Section V2',
      		$widget_options
    	);

		if ( is_active_widget(false, false, $this->id_base) ){
			add_action( 'wp_head', array(&$this, 'add_styles_and_scripts') );
		}
  	}

	function add_styles_and_scripts(){
		wp_enqueue_style( 'custom_section_v2', THEME_DIR_URI .'/dist/css/custom-section-v2.css', array(), '1', 'all' );
	}

	public function widget($args, $instance) {
		wp_enqueue_style( 'custom_section_v2', THEME_DIR_URI .'/dist/css/custom-section-v2.css', array(), '1', 'all' );
		if($instance){
			echo '
				<section class="custom_section_v2">
					<div class="container">
						<div class="section_intro">';
							echo $instance['section_title'] ? '<p class="section_title">'.$instance['section_title'].'</p>' : null;
							echo $instance['section_subtitle'] ? '<p class="section_subtitle">'.$instance['section_subtitle'].'</p>' : null ; echo '
						</div>
						<div class="row justify-content-between">
							<div class="col-lg-5">
								<div class="card card_left">
									<div class="image_wrapper">
										<img src="'.$instance['left_card_image'].'" alt="'.image_alt_by_url($instance['left_card_image']).'" class="clip-shield-svg">
										<svg class="svg">
											<clipPath id="shield-clip-path" clipPathUnits="objectBoundingBox">
												<path d="M0.999,0.001,1,0 H0 L0.001,0.001 V0.536 s-0.037,0.342,0.499,0.464 l0,0,0,0,0,0,0,0 c0.536,-0.121,0.499,-0.464,0.499,-0.464"></path>
											</clipPath>
										</svg>
									</div>
									<a href="'.$instance['left_card_btn_url'].'" class="btn btn-primary btn-icon-arrow_right">'.$instance['left_card_btn_text'].'</a>
								</div>
							</div>

							<div class="col-lg-5">
								<div class="card card_right">
									<div class="image_wrapper">
										<img src="'.$instance['right_card_image'].'" alt="'.image_alt_by_url($instance['right_card_image']).'" class="clip-shield-svg">
										<svg class="svg">
											<clipPath id="shield-clip-path" clipPathUnits="objectBoundingBox">
												<path d="M0.999,0.001,1,0 H0 L0.001,0.001 V0.536 s-0.037,0.342,0.499,0.464 l0,0,0,0,0,0,0,0 c0.536,-0.121,0.499,-0.464,0.499,-0.464"></path>
											</clipPath>
										</svg>
									</div>
									<a href="'.$instance['right_card_btn_url'].'" class="btn btn-primary btn-icon-arrow_right">'.$instance['right_card_btn_text'].'</a>
								</div>
							</div>
						</div>
					</div>
				</section>
			';
		}

	}

  	public function form($instance) {

		$right_card_btn_text = !empty($instance['right_card_btn_text']) ? $instance['right_card_btn_text'] : '';
		$right_card_btn_url = !empty($instance['right_card_btn_url']) ? $instance['right_card_btn_url'] : '';
		$left_card_btn_text = !empty($instance['left_card_btn_text']) ? $instance['left_card_btn_text'] : '';
		$left_card_btn_url = !empty($instance['left_card_btn_url']) ? $instance['left_card_btn_url'] : '';

		?>

		<h2>Section Intro</h2>

		<div class="customEditorParent">
			<label for="<?php echo $this->get_field_id('section_title'); ?>">Section Title</label>
			<textarea class="customEditor w-100" id="<?php echo $this->get_field_id('section_title'); ?>" name="<?php echo $this->get_field_name('section_title'); ?>"><?php echo $instance['section_title']; ?></textarea>
		</div>

		<div class="customEditorParent">
			<label for="<?php echo $this->get_field_id('section_subtitle'); ?>">Section Subtitle</label>
			<textarea class="customEditor w-100" id="<?php echo $this->get_field_id('section_subtitle'); ?>" name="<?php echo $this->get_field_name('section_subtitle'); ?>"><?php echo $instance['section_subtitle']; ?></textarea>
		</div>

		<h2>Left Card</h2>

		<p>
			<label for="<?php echo $this->get_field_id('left_card_btn_text'); ?>">Button Text</label>
			<input class="widefat" type="text" id="<?php echo $this->get_field_id('left_card_btn_text'); ?>" name="<?php echo $this->get_field_name('left_card_btn_text'); ?>" value="<?php echo esc_attr($left_card_btn_text); ?>" />
		</p>

		<p>
			<label for="<?php echo $this->get_field_id('left_card_btn_url'); ?>">Button URL</label>
			<input class="widefat" type="text" id="<?php echo $this->get_field_id('left_card_btn_url'); ?>" name="<?php echo $this->get_field_name('left_card_btn_url'); ?>" value="<?php echo esc_attr($left_card_btn_url); ?>" />
		</p>

		<p>
			<label for="<?php echo $this->get_field_id('left_card_image'); ?>">Left Card Image</label><br />
			<?php
				if ($instance['left_card_image'] != '') :
					echo '<img class="custom_media_image" src="' . $instance['left_card_image'] . '" style="margin:0;padding:0;max-width:100px;float:left;display:inline-block" /><br />';
				endif;
			?>
			<input type="text" class="widefat custom_media_url" name="<?php echo $this->get_field_name('left_card_image'); ?>" id="<?php echo $this->get_field_id('left_card_image'); ?>" value="<?php echo $instance['left_card_image']; ?>">
			<input type="button" class="button button-primary custom_media_button" id="custom_media_button" name="<?php echo 'button' + $this->get_field_name('left_card_image'); ?>" value="Upload Image" style="margin-top:5px;" />
		</p>

		<h2>Right Card</h2>

		<p>
			<label for="<?php echo $this->get_field_id('right_card_btn_text'); ?>">Button Text</label>
			<input class="widefat" type="text" id="<?php echo $this->get_field_id('right_card_btn_text'); ?>" name="<?php echo $this->get_field_name('right_card_btn_text'); ?>" value="<?php echo esc_attr($right_card_btn_text); ?>" />
		</p>

		<p>
			<label for="<?php echo $this->get_field_id('right_card_btn_url'); ?>">Button URL</label>
			<input class="widefat" type="text" id="<?php echo $this->get_field_id('right_card_btn_url'); ?>" name="<?php echo $this->get_field_name('right_card_btn_url'); ?>" value="<?php echo esc_attr($right_card_btn_url); ?>" />
		</p>

		<p>
			<label for="<?php echo $this->get_field_id('right_card_image'); ?>">right Card Image</label><br />
			<?php
				if ($instance['right_card_image'] != '') :
					echo '<img class="custom_media_image" src="' . $instance['right_card_image'] . '" style="margin:0;padding:0;max-width:100px;float:left;display:inline-block" /><br />';
				endif;
			?>
			<input type="text" class="widefat custom_media_url" name="<?php echo $this->get_field_name('right_card_image'); ?>" id="<?php echo $this->get_field_id('right_card_image'); ?>" value="<?php echo $instance['right_card_image']; ?>">
			<input type="button" class="button button-primary custom_media_button" id="custom_media_button" name="<?php echo 'button' + $this->get_field_name('right_card_image'); ?>" value="Upload Image" style="margin-top:5px;" />
		</p>

		<?php
	}

	public function update($new_instance, $old_instance) {
		$instance = $old_instance;

		$instance['section_title'] = $new_instance['section_title'];
		$instance['section_subtitle'] = $new_instance['section_subtitle'];

		$instance['left_card_btn_text'] = strip_tags($new_instance['left_card_btn_text']);
		$instance['left_card_btn_url'] = strip_tags($new_instance['left_card_btn_url']);
		$instance['left_card_image'] = strip_tags($new_instance['left_card_image']);

		$instance['right_card_btn_text'] = strip_tags($new_instance['right_card_btn_text']);
		$instance['right_card_btn_url'] = strip_tags($new_instance['right_card_btn_url']);
		$instance['right_card_image'] = strip_tags($new_instance['right_card_image']);

		return $instance;
  	}
}
?>