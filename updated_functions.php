...
// Parsing function (can be improved for complex structures)

<?php
/**
 * Splits a paragraph into the first sentence (title) and the rest (details).
 * It uses a regular expression that avoids splitting on a decimal point.
 *
 * @param string $paragraph
 * @return array  [title, details]
 */
function extractTitleAndDetails($paragraph) {
    // This regex splits on a period that is not part of a number:
    if (preg_match('/^(.*?)(?<!\d)\.(?!\d)(\s+|$)(.*)$/s', $paragraph, $matches)) {
        // Return first sentence (with period) and the remaining text
 ...
 *   where each item is given in two paragraphs (English then Arabic), with numbered lines.
 *
 * @param string $text
 * @return array
 */
function parse_newsletter($text) {
    $output = [
        'title'         => '',
        'introduction'  => [],
        'talks'         => []
        // Detailed sections (Economy, M&As, etc.) will be added later.
    ];
    ...
    foreach ($blocks as $block) {
        if (stripos($block, 'Podcast:') !== false) {
            // Remove the label "Podcast:" and trim the result
            $introText = trim(str_ireplace('Podcast:', '', $block));
            // Split into paragraphs (each separated by one or more newlines)
    ...
    // --- 3. Extract Top Talks ---
    // Look for the block that contains "Top Talks"
    $topTalksBlock = '';
    foreach ($blocks as $block) {
    ...
    if ($topTalksBlock !== '') {
        ...
        foreach ($lines as $line) {
            $line = trim($line);
            // Process only bullet lines
            if (strpos($line, '•') === 0) {
                // To pick the English ones, we assume no Arabic characters (Unicode range U+0600–U+06FF)
                if (!preg_match('/[\x{0600}-\x{06FF}]/u', $line)) {
                    // Remove the bullet
                    $lineContent = trim(ltrim($line, '•'));
                    // Example expected format:
                    // "Non-oil exports rose 17% Y-o-Y in 4Q24. (Economy) (1)"
                    if (preg_match('/^(.*?)(?<!\d)\.(?!\d)\s*\(([^)]+)\)\s*\((\d+)\)/', $lineContent, $matches)) {
                        $talkTitle    = trim($matches[1]).'.';
                        $talkCategory = trim($matches[2]);
                        $talkId       = (int)$matches[3];
                        $output['talks'][] = [
                            'title'    => $talkTitle,
    ...
    // These sections are labeled "Economy", "M&As", "Industries", "Geopolitical"
    $categories = ['Economy', 'M&As', 'Industries', 'Geopolitical'];
    foreach ($categories as $cat) {
        // Use a regex to capture the section content:
        if (preg_match('/' . preg_quote($cat, '/') . '\n(.*?)(?=\n(?:' . implode('|', $categories) . ')\n|$)/s', $text, $matches)) {
            $sectionText = trim($matches[1]);
            // Split the section into items using a pattern that matches numbered items
            // This pattern captures blocks that start with a number followed by a period.
            preg_match_all('/(^\d+\..*(?:\n(?!\d+\.).*)*)/m', $sectionText, $itemMatches);
            $itemsArray = $itemMatches[0];
            $sectionItems = [];
            // We expect items in pairs: English item followed by its Arabic version.
            for ($i = 0; $i < count($itemsArray); $i += 2) {
                $englishItem = isset($itemsArray[$i]) ? trim($itemsArray[$i]) : '';
                $arabicItem  = isset($itemsArray[$i+1]) ? trim($itemsArray[$i+1]) : '';
                
                // Remove the leading numbering (e.g. "1. ")
                $englishItem = preg_replace('/^\d+\.\s*/', '', $englishItem);
                $arabicItem  = preg_replace('/^\d+\.\s*/', '', $arabicItem);
                
                // Split into title and details (using our helper)
                list($titleEng, $detailsEng) = extractTitleAndDetails($englishItem);
                list($titleAra, $detailsAra) = extractTitleAndDetails($arabicItem);
